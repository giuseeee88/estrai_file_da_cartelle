...
parser = argparse.ArgumentParser(
    description="Estrae i file dalle sottocartelle della cartella indicata."
)
parser.add_argument(
    "cartella",
    nargs="?",
    help="Percorso della cartella principale contenente le sottocartelle.",
)
...
def selezionaCartella(mainLabel, mainCanvas, percorso_cli=None):
    parent_dir = percorso_cli or filedialog.askdirectory(
        initialdir="/",
        title="Selezionare la directory principale che contiene le varie sottodirectory"
    )
    ...
    if not os.path.isdir(parent_dir):
        print(f"Il percorso non è una cartella valida: {parent_dir}")
        return
    ...
    if not sottocartelle_paths:
        mostraPopupEstrazione(False, "Nessuna sottodirectory trovata nella directory selezionata.")
        return
...
def estraiFile(cartelleValide, componentiPreUpload, mainLabel, mainCanvas):
    ...
    if len(cartelleValide) > 0 and len(cartelleValide) < 1000:
        try:
            ...
            msgEstrazione = f"Errore nella creazione della cartella: {e}"
